...
"""
baseline_test.py
Baseline (normal) check:
 - Duration: 60 seconds
 - Target RPS: 5
 - Concurrency: 10
Output: results_baseline.csv + graphs
"""
...
def generate_graphs(csv_file, mode_name="baseline"):
    ...
    plt.savefig(f"{mode_name}_latency_over_time.png", dpi=150)
    plt.close()
    ...
    plt.savefig(f"{mode_name}_latency_histogram.png", dpi=150)
    plt.close()
    ...
    plt.savefig(f"{mode_name}_status_codes.png", dpi=150)
    plt.close()

    print(f"✅ Graphs saved for {mode_name} test")

def run(url, rps=5, duration_s=60, concurrency=10, out_csv="results_baseline.csv"):
    theoretical = int(rps * duration_s)
    max_requests = theoretical
    print(f"Baseline: {max_requests} requests to {url}")
    ...
    # Generate graphs
    generate_graphs(out_csv, "baseline")
