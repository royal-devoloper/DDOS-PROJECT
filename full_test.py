...
Full stress test:
 ...
 - Target RPS: 166  (≈99,600 requests, under 100k cap)
 - Concurrency: 200
Output: results_full.csv + graphs
"""
...
def generate_graphs(csv_file, mode_name="full"):
...
def run(url, rps=166, duration_s=600, concurrency=200, out_csv="results_full.csv"):
    theoretical = int(rps * duration_s)
    max_requests = min(theoretical, MAX_PROJECT_CAP)
    print(f"Full: {max_requests} requests to {url}")
    ...
    waf_block = status_counts.get(403,0) + status_counts.get(429,0)
    print(f"Possible WAF blocks (403/429): {waf_block}")
    if latencies:
        print(f"min/avg/max (s): {min(latencies):.4f}/{statistics.mean(latencies):.4f}/{max(latencies):.4f}")
    print(f"CSV -> {out_csv}")
    generate_graphs(out_csv, "full")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--url", default=DEFAULT_URL)
    parser.add_argument("--out", default="results_full.csv")
